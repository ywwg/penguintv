...
class OfflineImageCache:
	def __init__(self, store_location):
		...
		self._threadpool = ThreadPool.ThreadPool(5 ,"OfflineImageCache", daemon=True)
...
class UrlCacher:
	...
	def start_downloads(self):
		for url in self._cache_status.keys():
			if self._cache_status[url][1] != UrlCacher.DOWNLOADED:
				logging.debug("downloading image for cache: %s " % url)
	...
	def _download_image(self, args, retry=0):
		...
		try:
			...
			logging.debug("download failed %s" % url)
