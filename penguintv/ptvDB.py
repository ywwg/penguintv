...
(
	...
	def _process_feed(self,feed_id, args, total, data, recurse=0):# recurse=0):
		...
		if result>0:
			...
			if self.is_feed_filter(feed_id, c):
				...
				update_data['pollfail']=self.get_feed_poll_fail(self._resolve_pointed_feed(feed_id,c),c)
			else:
				self._db_execute(c, u'SELECT read FROM entries WHERE feed_id=?',(feed_id,))
				list = c.fetchall()
	...
	def poll_feed(self, feed_id, arguments=0, db=None, preparsed=None):
		...
		if preparsed is None:
			...
			self._db_execute(c, u'SELECT feed_pointer FROM feeds WHERE id=?',(feed_id,))
			result = c.fetchone()
			if result:
				if result[0] >= 0:
					return 0
				
			self._db_execute(c, """SELECT url,modified,etag FROM feeds WHERE id=?""",(feed_id,))
			data = c.fetchone()
			url,modified,etag=data
			try:
				feedparser.disableWellFormedCheck=1  #do we still need this?  it used to cause crashes
		...
		try:
			self._db_execute(c, u'SELECT title FROM feeds WHERE id=?',(feed_id,))
			exists=c.fetchone()
			if len(exists[0])>4:
				if exists[0][0:4]!="http": #hack to detect when the title hasn't been set yet because of first poll
				 	self._db_execute(c, """UPDATE feeds SET description=?, modified=?, etag=? WHERE id=?""", (channel['description'], modified,data['etag'],feed_id))
				else:
					self._db_execute(c, """UPDATE feeds SET title=?, description=?, modified=?, etag=? WHERE id=?""", (channel['title'],channel['description'], modified,data['etag'],feed_id))
		...
		if link is not None:
			...
			if link == "" and data['feed'].has_key('link'):
				self._db_execute(c, u'UPDATE feeds SET link=? WHERE id=?',(data['feed']['link'],feed_id))
		db.commit()
		
		#populate the entries
		self._db_execute(c, """SELECT id,guid,link,title,description FROM entries WHERE feed_id=? order by fakedate""",(feed_id,)) 
		...
		self._db_execute(c, """SELECT entries.id FROM entries INNER JOIN media ON entries.id = media.entry_id WHERE media.download_status>0 AND entries.feed_id=?""",(feed_id,))
		for id in c.fetchall():
			self._db_execute(c, """UPDATE entries SET old=0 WHERE id=?""",(id[0],))
		db.commit()
		#anything not set above as new, mod, or exists is no longer in
		#the xml and therefore should be deleted
		
		self._db_execute(c, """SELECT id FROM entries WHERE feed_id=?""",(feed_id,))
		all_entries = len(c.fetchall())
		self._db_execute(c, """SELECT id FROM entries WHERE old=1 AND feed_id=?""",(feed_id,))
		...
		if old_entries>0:
			...
			if MAX_ARTICLES > 0: #zero means never delete
				if new_entries >= MAX_ARTICLES:
					...
					self._db_execute(c, """DELETE FROM entries WHERE old=1 AND feed_id=?""",(feed_id,))
					...
					if old_articles_to_keep > 0:
						...
						self._db_execute(c, """SELECT id,title FROM entries WHERE old=1 AND feed_id=? ORDER BY fakedate LIMIT ?""",(feed_id,old_articles_to_ditch))
						ditchables = c.fetchall()
						for e in ditchables:
							self._db_execute(c, """DELETE FROM entries WHERE id=?""",(e[0],))
		self._db_execute(c, "DELETE FROM entries WHERE fakedate=0 AND feed_id=?",(feed_id,))
	...
	def _set_new_update_freq(self, db,c, feed_id, new_items):
		...
		self._db_execute(c, u'SELECT lastpoll, newatlast, pollfreq FROM feeds WHERE id=?',(feed_id,))
		...
		self._db_execute(c, u'UPDATE feeds SET lastpoll=?, newatlast=? WHERE id=?',(cur_time,new_items,feed_id))
		db.commit()
		...
		if new_items==0:
			...
			self._db_execute(c, 'SELECT date FROM entries WHERE feed_id=?',(feed_id,))
			datelist = c.fetchall()
	...
	def get_entry_media(self, entry_id, c=None):
		if c==None:
			c = self._c
		self._db_execute(c, """SELECT id,entry_id,url,file,download_status,viewed,length,mimetype FROM media WHERE entry_id = ?""",(entry_id,))
		dataList=c.fetchall()
	...
	def get_entrylist(self, feed_index, c=None):
		if c is None:
			c = self._c
		self._db_execute(c, u'SELECT feed_pointer,description FROM feeds WHERE id=?',(feed_index,))
		result = c.fetchone()
		if result is None:
			return []
		if result[0] >= 0:
			...
			#need list of "new"s.
			#self._db_execute(c, u'SELECT id,new FROM entries WHERE feed_id=? ORDER BY fakedate DESC',(pointed_feed,))
			#new_info = c.fetchall()
			#ret_val = []
			#i=0
			#try:
		...
		self._db_execute(c, """SELECT id,title,fakedate FROM entries WHERE feed_id=? ORDER BY fakedate DESC""",(feed_index,))
		result = c.fetchall()
	...
	def get_media_for_download(self):
		self._db_execute(self._c, u'SELECT media.id, media.length, media.entry_id, entries.feed_id FROM media INNER JOIN entries ON media.entry_id = entries.id WHERE (download_status==? OR download_status==?) AND viewed=0',(D_NOT_DOWNLOADED,D_RESUMABLE))
		list=self._c.fetchall()
		self._db_execute(self._c, u'SELECT media.id, media.length, media.entry_id, entries.feed_id FROM media INNER JOIN entries ON media.entry_id = entries.id WHERE download_status==?',(D_ERROR,))
	...
	def get_resumable_media(self):
		self._db_execute(self._c, u'SELECT media.id, media.file, media.entry_id, entries.feed_id  FROM media INNER JOIN entries ON media.entry_id = entries.id WHERE download_status=?',(D_RESUMABLE,))
		list = self._c.fetchall()
		dict_list = []	
		dict = {}
		for item in list:
	...
	def mark_feed_as_viewed(self, feed_id):
		...
		if self._filtered_entries.has_key(feed_id):
			...
			for entry in self._filtered_entries[feed_id]:
				self._db_execute(self._c, u'UPDATE entries SET read=1 WHERE id=?',(entry[0],))
				self._db_execute(self._c, u'SELECT media.id, media.download_status FROM media WHERE entry_id=?',(entry[0],))
				list = list+self._c.fetchall()
			feed_id = self._resolve_pointed_feed(feed_id)
		else:
			#feed_id = self._resolve_pointed_feed(feed_id)
			self._db_execute(self._c, u'UPDATE entries SET read=1 WHERE feed_id=?',(feed_id,))#
			self._db_execute(self._c, u'SELECT media.id, media.download_status FROM media INNER JOIN entries ON media.entry_id = entries.id WHERE entries.feed_id = ?',(feed_id,))
			list = self._c.fetchall()
			
		for item in list:
			self._db_execute(self._c, u'UPDATE media SET viewed=? WHERE id=?',(1,item[0]))
			if item[1] == D_ERROR:
	...
	def media_exists(self, filename):
		self._db_execute(self._c, u'SELECT media.id FROM media WHERE media.file=?',(filename,))
		list=self._c.fetchall()
		if len(list)>1:
	...
	def get_entry_download_status(self, entry_id, c=None):
		if c == None:
			c = self._c
		self._db_execute(c, u'SELECT media.download_status, media.viewed FROM media INNER JOIN entries ON media.entry_id=entries.id WHERE media.download_status!=0 AND entries.id=?',(entry_id,))
		result = c.fetchall()
		#if entry_id==262:
		#	print result
	...
	def get_feed_poll_fail(self, feed_id,c=None):
		...
		self._db_execute(c, u'SELECT pollfail FROM feeds WHERE id=?',(feed_id,))
		result = c.fetchone()[0]
		if result==0:
			return False
		return True
	...
	def get_entry_flag(self, entry_id, c=None, medialist=None):
		...
		self._db_execute(c, u'SELECT read FROM entries WHERE id=?',(entry_id,))
		read = c.fetchone()[0]
	...
	def get_unread_count(self, feed_id, c=None):
		...
		if self._filtered_entries.has_key(feed_id):
			...
			list = []
			for entry in entries:
				self._db_execute(c, u'SELECT read FROM entries WHERE id=?',(entry[0],))
				try:
	...
	def get_entry_flags(self, feed_id, c=None):
		if c is None:
			c = self._c
		#self._db_execute(c, u'SELECT feed_pointer, description FROM feeds WHERE id=?',(feed_id,))
		#result = c.fetchone()
		#if result[0] >= 0:
		if self._filtered_entries.has_key(feed_id):
			...
			self._db_execute(c, u'SELECT id FROM entries WHERE feed_id=?',(feed_id,))
	...
	def maybe_write_term_frequency_table(self):
		try:
			self._db_execute(self._c, u'SELECT value FROM settings WHERE data="frequency_table_update"')
			last_update = self._c.fetchone()
			if last_update is None:
				print "nothing there"
				self._db_execute(self._c, u'INSERT INTO settings (data, value) VALUES ("frequency_table_update",?)',(time.time(),))
				self._db.commit()
				return
	...
	def write_term_frequency_table(self):
		...
		#for t in terms:
		#	i+=1
		#	self._db_execute(self._c, u'INSERT INTO terms (term, frequency) VALUES (?,?)',(t[0],t[1]))
		#self._db_execute(self._c, u'UPDATE settings SET value=? WHERE data="frequency_table_update"',(time.time(),))
		#self._db.commit()
	...
	def get_recent_popular_terms(self):
		...
		#print old_terms
		#self._db_execute(self._c, u'SELECT term,frequency FROM terms')	
		#old_terms = self._c.fetchall()
	...
	def get_pointer_feeds(self, feed_id, c=None):
		if c is None:
			c = self._c
		self._db_execute(c, u'SELECT id FROM feeds WHERE feed_pointer=?',(feed_id,))
		results = c.fetchall()
		if results is None:
			return []
