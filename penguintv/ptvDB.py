...
(
	...
	def _polling_callback(self, data, cancelled=False):
	...
	def poll_feed(self, feed_id, arguments=0, preparsed=None):
		...
		for item in data['items']:
			...
			if item.has_key('title') == 0:
				item['title'] = ""
			if item['title']=="":
				item['title']=item['description'][0:100]
				...
				if html_begin >= 0 and html_begin < 5: #in case it _begins_ with html, and the html is really early
					...
					item['title'] = STRIPPER_REGEX.sub('', desc)[:100]
				elif html_begin > 5: #in case there's html within 100 chars...
	...
	def mark_feed_as_viewed(self, feed_id):
		...
		while len(list) > 0:
			length = min(len(list), 256)
			qmarks = "?,"*(length-1)+"?"
			idlist = [l[0] for l in list[0:length]]
